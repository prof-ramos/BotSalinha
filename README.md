...
- 🧭 **Vector DB Dedicado (Qdrant)**: Backend vetorial opcional para escalar busca semântica
...
| `RAG__VECTOR_BACKEND`        | `sqlite`                       | Backend vetorial (`sqlite` ou `qdrant`)    |
| `RAG__QDRANT_URL`            | `http://localhost:6333`        | URL do serviço Qdrant                       |
| `RAG__QDRANT_COLLECTION`     | `botsalinha_chunks`            | Nome da collection no Qdrant                |
...
### Vector DB dedicado (Qdrant)

Para habilitar backend vetorial dedicado:

```env
RAG__VECTOR_BACKEND=qdrant
RAG__QDRANT_URL=http://localhost:6333
RAG__QDRANT_COLLECTION=botsalinha_chunks
# Opcional:
# RAG__QDRANT_API_KEY=seu_token
```

Com isso, a ingestão envia embeddings para o Qdrant e as consultas RAG passam a buscar no vector DB dedicado.
